...
def index():
    return 'Simple web API for newsreader (at /newsreader; <a href="/newsreader?text=Geef mij maar Amsterdam">example</a>) and alpino (at /parse; <a href="/parse?text=dit is een test">example</a>). See <a href="github.com/vanatteveldt/newsreader-docker">vanatteveldt/newsreader-docker</a> for more information.', 200
...
@app.route('/newsreader', methods=['GET'])
def newsreader_get():
    text = request.args.get('text', None)
    if not text:
        return "Usage: /newsreader?text=text_to_parse", 400
    result = newsreader(text)
    return Response(result, mimetype='text/xml')

@app.route('/newsreader', methods=['POST'])
def newsreader_post():
    body = request.get_json(force=True)
    text = body['text']
    result = newsreader(text)
...
def alpino_dependencies(tokens):
    """
...
def read_triples_into_dict(triples, result, strip_id=False):
...
def alpino_treebank_triples(input):
    with tempfile.TemporaryDirectory(prefix="alpinoserver-") as d:
...
def alpino_xml_raw(input, treebank):
